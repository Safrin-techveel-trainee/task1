 const escalationtimes= [
         {
             ...
             "priority": "HIGH",
             "level": 2, // level 1,timelimit = active ,level 2=inactive
             "timeLimit": 750
         },
         {
...
let updatedApiData=apiData.map(apiItem=>{
    let escalationTime=escalationtimes.find(et=>et.roleId===apiItem.roleId);
    if (escalationTime) {
        if (apiItem.level===escalationTime.level) {
            if (apiItem.level===1 && escalationTime.timeLimit) {
                return{...apiItem,status:"ACTVE"};
            }else if(apiItem.level===2 && escalationTime.timeLimit){
                return{...apiItem,status:"IN-ACTIVE"}
            }else if(apiItem.level===1 && apiItem.level===2 && escalationTime.timeLimitApporver && escalationTime.timeLimitRiewer){
                return{...apiItem,status:"ACTIVE"}
            }
        }
    }
    return apiItem;
